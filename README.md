...
持久化状态和业务枚举在数据库及 HTTP/Swagger 契约中统一使用数字：二值状态 `0` 表示否、隐藏、未删除或非系统，`1` 表示是、显示、已删除或系统；菜单类型为目录 `1`、页面 `2`、按钮 `3`；聊天消息角色为用户 `1`、助手 `2`。天气日期、SSE 事件和 Agent 配置等非持久化协议值继续使用字符串。
